...
} from "@/components/employee/RequestLeaveModal";
...
export function ProfileLeaveTab({
  ...
  viewerRole: "self" | "reviewer";
  balances?: LeaveBalance[];
  history: LeaveHistoryItem[];
  leaveTypeOptions?: LeaveTypeOption[];
  onRequestLeave?: (data: LeaveRequestInput) => void | Promise<void>;
  ...
  async function handleRequestSubmit(data: LeaveRequestInput) {
    await onRequestLeave?.(data);
    setModalOpen(false);
  }

  return (
    <div className="flex flex-col gap-6">
      {viewerRole === "self" && balances && (
            ...
            <TableBody>
              {history.map((h) => (
                <TableRow key={h.id}>
                  ...
                  <TableCell>
                    {h.status === "Pending" && viewerRole === "reviewer" ? (
                    ...
                    ) : h.status === "Pending" && viewerRole === "self" ? (
                      <Button
                        size="sm"
                        variant="outline"
